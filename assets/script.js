...
function handleAdminAuth(e){
  ...
  if(u==='admin' && p==='password'){
    localStorage.setItem('church_admin', '1');
    window.location.href = 'admin.html';
  } else {
    n.style.display='block'; n.textContent='Invalid login';
  }
  return false;
}

// Admin.html access check
...
function renderGallery(){ 
  ...
  const posts=getPosts().filter(p=>p.type==='gallery'); g.innerHTML='';
  posts.reverse().forEach(p=>{
    const d=document.createElement('div'); d.className='card'; 
    d.style.minHeight='110px'; d.style.display='flex'; d.style.alignItems='center'; d.style.justifyContent='center';
    if(p.img){ d.innerHTML=`<img src="${p.img}" alt="${p.title}" style="max-width:100%;border-radius:8px;">`; } 
    else { d.textContent=p.title; } 
    g.appendChild(d); 
  }); 
}
...
function renderSermons(){ 
  const container = document.getElementById('sermonGroups'); if(!container) return; 
  const posts = getPosts().filter(p=>p.type==='sermon'); 
  const groups = {}; 
  posts.forEach(p=>{ 
  ...
  Object.keys(groups).sort((a,b)=>b-a).forEach(year=>{
    const yBlock=document.createElement('div'); yBlock.innerHTML=`<h3>${year}</h3>`; 
    Object.keys(groups[year]).forEach(month=>{
      const mBlock=document.createElement('div'); mBlock.innerHTML=`<h4>${month}</h4>`; 
      groups[year][month].forEach(p=>{
        const item=document.createElement('div'); 
        item.className='card'; item.style.marginTop='0.5rem'; 
        item.innerHTML=`<strong>${p.title}</strong><br><div class='small muted'>${p.date.slice(0,10)}</div><div class='muted'>${p.content||''}</div>`;
